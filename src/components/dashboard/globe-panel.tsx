...
export function GlobePanel() {
  ...
  const { state, ask, reset } = useAgentChat();
  ...
  return (
    <div className="panel panel-hot relative h-[560px] overflow-hidden">
      ...
      {/* Ação corrente do agente — vira a resposta do chat quando há uma pergunta ativa */}
      <div className="absolute inset-x-0 bottom-[86px] flex justify-center px-5">
        {state.status === "idle" ? (
          <div className="pointer-events-none tile flex max-w-[92%] items-center gap-2.5 px-3.5 py-2 backdrop-blur-xl">
          ...
          <div className="tile max-w-[92%] px-3.5 py-2.5 backdrop-blur-xl">
            <div className="flex items-start gap-2.5">
              {state.status === "loading" ? (
                <Loader2
                  className="mt-0.5 h-3.5 w-3.5 shrink-0 animate-spin text-brand-400"
                  strokeWidth={2}
                />
              ) : (
                <Sparkles
                  className={cn(
                    ...
                    state.status === "error" ? "text-caution" : "text-brand-400",
                  )}
                  strokeWidth={2}
                />
              )}
              <div className="min-w-0">
                <p className="text-[11px] text-fg-ghost">Você: {state.question}</p>
                <p
                  className={cn(
                    "mt-0.5 text-[12.5px] leading-relaxed",
                    state.status === "error" ? "text-caution" : "text-fg",
                  )}
                >
                  {state.status === "loading"
                    ? "Pensando…"
              ...
              {state.status !== "loading" && (
                <button
                  onClick={reset}
                  className="ml-auto shrink-0 text-[10.5px] text-fg-ghost hover:text-fg-muted"
                >
                  limpar
                </button>
              )}
      ...
      {/* Entrada de comando */}
      <form onSubmit={handleSubmit} className="absolute inset-x-0 bottom-0 p-4">
        <div className="flex items-center gap-2 rounded-full border border-hairline bg-ink/80 px-2 py-1.5 backdrop-blur-2xl">
          <input
            ...
            placeholder="Pergunte ao agente sobre a operação… (teste real, custa uso de API)"
            maxLength={500}
          ...
          <button
            ...
            className="btn-primary !h-8 !w-8 !p-0 disabled:opacity-40"
          >
